 ...
 *
 * Copyright (C) 2021-2024, Rivos Inc.
 *
...
#define PCI_SEG_NUM(devid)            ((devid) >> 16)
...
struct RISCVIOMMUSpace {
    ...
    PCIDevice *pdev;            /* associated PCI device pointer */
...
static void riscv_iommu_notify(RISCVIOMMUState *s, int vec)
{
    ...
    if (!(ipsr & (1 << vec))) {
	qemu_irq_pulse(s->irqs[(ivec >> (vec * 4)) & 0x0F]);
    }
}
...
static void riscv_iommu_fault(RISCVIOMMUState *s, struct riscv_iommu_fq_record *ev)
{
    ...
    trace_riscv_iommu_flt(s->parent_obj.id, PCI_SEG_NUM(devid), PCI_BUS_NUM(devid),
                          PCI_SLOT(devid), PCI_FUNC(devid), ev->hdr, ev->iotval);
    ...
    unsigned size, MemTxAttrs attrs)
{
    ...
    case RISCV_IOMMU_MSI_PTE_M_BASIC:
        ...
        trace_riscv_iommu_msi(s->parent_obj.id, PCI_SEG_NUM(ctx->devid), PCI_BUS_NUM(ctx->devid),
...
static int riscv_iommu_ctx_fetch(RISCVIOMMUState *s, RISCVIOMMUContext *ctx)
{
    ...
    for (depth = mode - RISCV_IOMMU_DC_FSC_PDTP_MODE_PD8; depth-- > 0; ) {
        ...
        if (de & ~(RISCV_IOMMU_PDTE_PPN | RISCV_IOMMU_PDTE_VALID)) {
            return RISCV_IOMMU_FQ_CAUSE_PDT_INVALID; /* reserved bits set. */
        }
...
static AddressSpace *riscv_iommu_space(RISCVIOMMUState *s, uint32_t devid, PCIDevice *pdev)
{
    RISCVIOMMUSpace *as;

    /* Ignore segment number */
    if (BIT_ULL(RISCV_IOMMU_DISABLE_PCI_SEGMENT) & s->disable) {
        devid = devid & 0xffff;
    }

    qemu_mutex_lock(&s->core_lock);
    QLIST_FOREACH(as, &s->spaces, list) {
        if (as->devid == devid && (!pdev || pdev == as->pdev)) {
            break;
        }
    }
    ...
    if (as == NULL) {
        ...
        snprintf(name, sizeof(name), "riscv-iommu-%04x:%02x:%02x.%d-iova",
        ...
        trace_riscv_iommu_new(s->parent_obj.id, PCI_SEG_NUM(as->devid),
    ...
    IOMMUTLBEntry *iotlb, bool enable_cache)
{
    ...
    enable_cache &= !(BIT_ULL(RISCV_IOMMU_DISABLE_IOTLB) & s->disable);
    ...
    if (iotlb->perm == IOMMU_NONE) {
        ...
        trace_riscv_iommu_ats(s->parent_obj.id, PCI_SEG_NUM(ctx->devid),
                PCI_BUS_NUM(ctx->devid), PCI_SLOT(ctx->devid), PCI_FUNC(ctx->devid),
                iotlb->iova);
    }
    ...
    if (!fault && iotlb->addr_mask != ~TARGET_PAGE_MASK) {
        /* Do not cache huge page translations (2M, 1G, ...) */
        goto done;
    }
...
static void riscv_iommu_iofence_actions(RISCVIOMMUState *s)
{
    RISCVIommuIofenceCallback *cb;
    QLIST_FOREACH(cb, &s->iofence_callbacks, node) {
        int res = (cb->fn)(cb->opaque);
        if (res) {
            warn_report("Error in iofence action");
        }
    }
}
    ...
    void (*trace_fn)(const char *id))
{
    ...
    switch (flag) {
    case IOMMU_NOTIFIER_DEVIOTLB_UNMAP: {
        /*
         * Encode the address according to the section 10.2.3.2 of the PCIe
         * spec. The page mask is encoded in the S bit and consecutive LSBs
         * of the address being set to 1.
         */
        uint8_t s_bit = get_field(cmd->dword1, RISCV_IOMMU_CMD_ATS_INVAL_S);
        uint64_t addr = get_field(cmd->dword1, RISCV_IOMMU_CMD_ATS_INVAL_UADDR);
        event.entry.addr_mask = 0xfff;
        if (s_bit) {
            event.entry.addr_mask = 0x1fff;
            for (uint8_t i = 0; i < 64; ++i) {
                if (!(addr & (1ull << i))) {
                    break;
                }
                event.entry.addr_mask |=
                    (1ull << (i + RISCV_IOMMU_CMD_ATS_INVAL_PAGE_BITS + 1));
            }
        }
        event.entry.iova =
            (addr << RISCV_IOMMU_CMD_ATS_INVAL_PAGE_BITS) &
            ~event.entry.addr_mask;
        break;
    }
    case IOMMU_NOTIFIER_MAP: {
        unsigned resp_code = get_field(cmd->dword1,
                                       RISCV_IOMMU_CMD_ATS_PRGR_RESP_CODE);
        group_idx = get_field(cmd->dword1, RISCV_IOMMU_CMD_ATS_PRGR_PRG_INDEX);
        event.entry.perm = resp_code ? IOMMU_NONE : IOMMU_RW;
        break;
    }
    default:
        break;
    ...
    IOMMU_NOTIFIER_FOREACH(n, &as->iova_mr) {
        if ((!pv || n->iommu_idx == pasid) && (n->notifier_flags & flag)) {
            if (flag == IOMMU_NOTIFIER_MAP) {
                /* Embedding the group index inside the IOVA */
                event.entry.iova = set_field(n->start,
                                             RISCV_IOMMU_PREQ_PRG_INDEX,
                                             group_idx);
...
{
    return riscv_iommu_ats(s, cmd, IOMMU_NOTIFIER_DEVIOTLB_UNMAP,
...
static void riscv_iommu_ats_prgr(RISCVIOMMUState *s,
    struct riscv_iommu_command *cmd)
{
    return riscv_iommu_ats(s, cmd, IOMMU_NOTIFIER_MAP,
...
static void riscv_iommu_process_ddtp(RISCVIOMMUState *s)
{
    ...
    /* Check if new mode is allowed. */
    if ((new_mode == RISCV_IOMMU_DDTP_MODE_1LVL) && (BIT_ULL(RISCV_IOMMU_DISABLE_DDT_1LVL) & s->disable))
        ok = false;
    if ((new_mode == RISCV_IOMMU_DDTP_MODE_2LVL) && (BIT_ULL(RISCV_IOMMU_DISABLE_DDT_2LVL) & s->disable))
        ok = false;
    if ((new_mode == RISCV_IOMMU_DDTP_MODE_3LVL) && (BIT_ULL(RISCV_IOMMU_DISABLE_DDT_3LVL) & s->disable))
        ok = false;
    ...
    if (ok && !(BIT_ULL(RISCV_IOMMU_DISABLE_DDT_FLUSH) & s->disable)) {
        /* Clear IOTLB/IOCTX cache on DDT update */
        riscv_iommu_iot_inval(s, __iot_inval_all, 0, 0, 0);
        riscv_iommu_ctx_inval(s, __ctx_inval_all, 0, 0);
    }
    ...
    while (tail != head) {
        ...
        case RISCV_IOMMU_CMD(RISCV_IOMMU_CMD_IOFENCE_FUNC_C,
            ...
            res = riscv_iommu_iofence(s, cmd.dword0 & RISCV_IOMMU_CMD_IOFENCE_AV,
                cmd.dword1 << RISCV_IOMMU_CMD_IOFENCE_ADDR_SHIFT,
...
static void riscv_iommu_realize(DeviceState *dev, Error **errp)
{
    ...
    QLIST_INIT(&s->iofence_callbacks);
    ...
    s->iommu_space = riscv_iommu_space;
}
...
static void riscv_iommu_unrealize(DeviceState *dev)
{
    ...
    pthread_rwlock_destroy(&s->iot_lock);
...
static Property riscv_iommu_properties[] = {
    ...
    DEFINE_PROP_UINT64("disable", RISCVIOMMUState, disable, 0),
    ...
    DEFINE_PROP_LINK("iommu-bus", RISCVIOMMUState, iommu_bus,
        TYPE_PCI_BUS, PCIBus *),
    ...
    IOMMUAccessFlags flag, int iommu_idx)
{
    ...
    trace_riscv_iommu_dma(as->iommu->parent_obj.id, PCI_SEG_NUM(as->devid),
                          PCI_BUS_NUM(as->devid), PCI_SLOT(as->devid), PCI_FUNC(as->devid),
                          iommu_idx, IOMMU_FLAG_STR[flag & IOMMU_RW], iotlb.iova,
...
static AddressSpace *riscv_iommu_find_as(PCIBus *bus, void *opaque, int devfn)
{
    ...
    uint32_t devid = PCI_BUILD_BDF(bus_num, devfn);
    AddressSpace *as = NULL;

    /* Bypass unknown PCI devices and IOMMUs */
    PCIDevice *pdev = pci_find_device(bus, bus_num, devfn);
    ...
    while (s != NULL && as == NULL) {
        as = riscv_iommu_space(s, devid, pdev);
        s = s->iommus.le_next;
    }
