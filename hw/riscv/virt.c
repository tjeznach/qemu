...
static void create_fdt_iommu(RISCVVirtState *s, const MemMapEntry *memmap, uint32_t irq_mmio_phandle)
{
    MachineState *ms = MACHINE(s);
...
static DeviceState *virt_create_iommu(RISCVVirtState *s, DeviceState *irqchip)
{
    ...
    /* Disable MSI_FLAT [22], MSI_MRIF [23] */
    ...
    /* enable pass-through at reset */
    qdev_prop_set_bit(iommu, "off", FALSE);
