...
typedef struct RISCVIommuIofenceCallback RISCVIommuIofenceCallback;
...
struct RISCVIOMMUState {
    ...
    uint64_t disable;     /* IOMMU disabled features, implementation details */
    ...
    AddressSpace *(*iommu_space)(RISCVIOMMUState *s, uint32_t devid, PCIDevice *pdev);

    /* Targets for iofence signaling */
    QLIST_HEAD(, RISCVIommuIofenceCallback) iofence_callbacks;

    QLIST_ENTRY(RISCVIOMMUState) iommus;
    QLIST_HEAD(, RISCVIOMMUSpace) spaces;
};

/* iofence callback handling */
typedef int (*RISCVIommuIofenceCallbackFn)(void *opaque);
struct RISCVIommuIofenceCallback {
    RISCVIommuIofenceCallbackFn fn;
    void *opaque;
    QLIST_ENTRY(RISCVIommuIofenceCallback) node;
};

static inline void riscv_iommu_iofence_register_action(
    RISCVIOMMUState *s,
    RISCVIommuIofenceCallbackFn fn,
    void *opaque) {
    RISCVIommuIofenceCallback *cb =
        g_malloc0(sizeof(RISCVIommuIofenceCallback));
    cb->fn = fn;
    cb->opaque = opaque;
    QLIST_INSERT_HEAD(&s->iofence_callbacks, cb, node);
}
...
typedef struct RISCVIOMMUStatePci {
    ...
    bool             ea;      /* MMIO via EA instead of BAR0 */
